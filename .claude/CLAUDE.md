# BASRaT Revision Assistant

A Next.js application that helps sport rehabilitation students prepare for the BASRaT registration exam using AI-powered MCQ generation and spaced repetition (SM-2 algorithm).

## Project Stack

- **Framework**: Next.js 14 (App Router)
- **Auth & DB**: Supabase (email + Google OAuth, Row Level Security)
- **AI**: Anthropic Claude API (`@anthropic-ai/sdk`) — question generation via `claude-opus-4-6`
- **Styling**: Tailwind CSS
- **Spaced Repetition**: SM-2 algorithm in `lib/sm2.ts`

## Key Directories

- `app/api/` — API routes (generate-questions, process-document, questions, progress, sessions, exam-date)
- `app/(app)/` — Authenticated pages (dashboard, topic quiz)
- `app/(auth)/` — Auth pages (login)
- `components/` — React components (dashboard, quiz, heatmap, auth)
- `lib/` — Utilities (claude.ts, sm2.ts, localStorage.ts, supabase/)
- `data/topics.ts` — 22 pre-built exam topics
...
See `.env.example` for required variables (Supabase URL/keys, Anthropic API key).

---

# Academic Research Skills

A suite of Claude Code skills for rigorous academic research, paper writing, peer review, and pipeline orchestration. Skills are located in `.claude/skills/academic-research-skills/`.
...
- **Cross-model verification** (optional): Set `ARS_CROSS_MODEL` env var to enable GPT-5.4 Pro or Gemini 3.1 Pro for integrity sample checks and independent Devil's Advocate critique. See `shared/cross_model_verification.md`.
