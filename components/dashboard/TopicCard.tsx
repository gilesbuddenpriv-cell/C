...
export default function TopicCard({ topic, isNew = true, hasProgress = false }: TopicCardProps) {
  return (
    <div className="card relative group hover:shadow-sm hover:border-forest/30 transition-all">
      ...
      {/* Main link */}
      <Link href={`/topic/${topic.slug}`} className="block p-4 pb-2">
      ...
      <div className="flex border-t border-border">
        <Link
          ...
          className="flex-1 text-center py-2 text-xs font-mono font-bold text-muted hover:text-forest hover:bg-white transition-colors border-r border-border"
          style={{ fontSize: '0.6rem', letterSpacing: '0.08em' }}
        >
          MCQ
        </Link>
        <Link
          href={`/topic/${topic.slug}/flash`}
          className="flex-1 text-center py-2 text-xs font-mono font-bold text-muted hover:text-forest hover:bg-white transition-colors"
          style={{ fontSize: '0.6rem', letterSpacing: '0.08em' }}
        >
          🎴 FLASH
        </Link>
